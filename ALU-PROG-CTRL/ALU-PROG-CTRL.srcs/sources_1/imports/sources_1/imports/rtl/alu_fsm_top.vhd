...
-- Dev:         Jose Quinteros del Castillo y Lorenzo Cabrera
--
-- Design Name:
-- Module Name:
-- Project Name:
...
architecture rtl of alu_fsm_top is
  ...
  signal program_step : std_logic;
  signal op_en_alu : std_logic;
...
begin
  ...
  input_conditioning_inst : entity work.input_conditioning
    ...
    port map(
      ...
      op_addr0_out   => open,
      op_addr1_out   => open,
      op_en_out      => program_step -- PASE DE OP_EN -> PROGRAM_STEP
                                     -- el op_en hace avanzar el programa en un paso
    );

      -- Program Control
  program_control_inst :entity work.program_control
    port map(
    sys_clk_in => sys_clk_in,
    sys_rst_in =>sys_rst,

    program_step_in => program_step,

    op_en_out  => op_en_alu, -- La salida de esto indica a la ALU cuando trabajar  
    op_sel_out => op_sel,
